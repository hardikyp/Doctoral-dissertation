...
This section extends the Three-Node Torsional Spring (3NTS) element from Chapter~\ref{chapter-03} by introducing a constitutive law with prescribed angular limits. The enhanced law preserves the linear spring response within the intended operating range and introduces nonlinear stiffening near the deformation limits. This formulation prevents inadmissible hinge rotations and enables the analysis of sequential folding, locking, and load redistribution in reconfigurable trusses.
...
\begin{figure}[!ht]
  ...
  \caption{Deployment-path analysis of reconfigurable trusses using the enhanced torsional spring law: (a) Vertically stacked square-linkage truss with two independent folding units under compression. Each unit contains an enhanced torsional spring at the midpoint of the diagonal member; (b) Load-displacement response showing sequential instability, folding, and locking of the lower and upper units. After each unit reaches its prescribed angular limit, the enhanced spring stiffens and redirects deformation to the remaining active degree of freedom; (c) Reconfigurable Warren truss configurations produced by folding different combinations of independent units; (d) Load-displacement responses for folding Warren truss configurations to the same compact state. The results show that the intermediate force demand depends on the folding path, even when the maximum force required to fold the entire structure is the same.}\label{fig-deployment-path}
\end{figure}
