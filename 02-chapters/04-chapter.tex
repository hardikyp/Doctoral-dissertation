...
Within the admissible hinge-angle interval \(\alpha_1 < \alpha < \alpha_2\), the bistable torsional spring is defined by prescribing its tangent stiffness as a smooth function of the hinge rotation. The rest angle \(\alpha_0\) denotes the central equilibrium configuration. The signed relative rotation is defined as \(\theta = \alpha - \alpha_0\). The constitutive law is assumed to be symmetric about \(\alpha_0\). Therefore, the tangent stiffness is written in terms of the rotation magnitude \(|\theta|\), while the sign of the restoring moment is recovered using \(\chi = \operatorname{sgn}(\theta)\).

The bistable response is constructed by prescribing a tangent stiffness that transitions from an initial positive stiffness, to a negative stiffness region, and finally to a positive stiffness region. The initial positive stiffness defines the response near the central equilibrium. The negative stiffness region creates an unstable segment in the moment-rotation response. This unstable segment produces the energy barrier between stable configurations. The final positive stiffness region restores stability after the hinge passes through the unstable segment.

The smooth transition between stiffness regimes is defined using the normalized function
\begin{flalign}
...
\end{flalign}
The parameter \(\beta\) controls the sharpness of the transition. Larger values of \(\beta\) produce sharper transitions between stiffness regimes, while smaller values produce more gradual transitions. The function satisfies \(q(0)=0\) and \(q(1)=1\), so it provides a smooth interpolation between two prescribed stiffness values.

Let \(K_T\) denote the initial positive stiffness, \( K_n \) denote the negative stiffness, and \( K_p \) denote the final positive stiffness. The transition rotations are denoted by \(\theta_1\), \(\theta_2\), and \(\theta_3\), where \(\theta_1 < \theta_2 < \theta_3\). The tangent stiffness in the bistable region is defined as
\begin{flalign}
\quad K_T(\theta)&=
\begin{cases}
K_T,
\qquad \qquad & 0 \le |\theta| \le \theta_1, \\[6pt]
K_T + (K_n - K_T)
q\left(\dfrac{|\theta|-\theta_1}{\theta_2-\theta_1}\right),
\qquad \qquad & \theta_1 < |\theta| \le \theta_2, \\[12pt]
K_n + (K_p - K_n)
q\left(\dfrac{|\theta|-\theta_2}{\theta_3-\theta_2}\right),
\qquad \qquad & \theta_2 < |\theta| \le \theta_3, \\[12pt]
K_p,
\qquad \qquad & |\theta| > \theta_3 .
\end{cases}&
\end{flalign}
This expression directly defines the desired stiffness landscape. The first branch gives a stable linear response near \(\alpha_0\). The second branch smoothly decreases the tangent stiffness from \(K_T\) to \(K_n\), thereby introducing a negative-stiffness regime. The third branch smoothly increases the tangent stiffness from \(K_n\) to \(K_p\), which restores positive stiffness and creates the second stable region.
...
\end{flalign}
This definition makes the moment antisymmetric about \(\alpha_0\), while the tangent stiffness remains symmetric about \(\alpha_0\).

The strain energy is obtained by integrating the restoring moment from the reference configuration to the current hinge angle:
\begin{flalign}
\quad U_B(\alpha)&=\int_{\alpha_0}^{\alpha}M_B(\tilde{\alpha})\,d\tilde{\alpha}~.&
\end{flalign}
Because the constitutive law is symmetric about \( \alpha_0 \), the energy can also be expressed in terms of the rotation magnitude as
\begin{flalign}
\quad U_B(\theta)&=\int_0^{|\theta|}M_{\mathrm{mag}}(\eta)\,d\eta~.&
\end{flalign}
Equivalently, since the moment is the integral of the tangent stiffness, the energy may be written as
\begin{flalign}
\quad U_B(\theta)&=\int_0^{|\theta|}\left[\int_0^\eta K_B(\xi) \ d\xi\right]d\eta~.&
\end{flalign}
This expression emphasizes that the bistable energy landscape is generated directly from the prescribed tangent stiffness. The positive-stiffness regions form stable wells, while the negative-stiffness region creates the energy barrier separating them.

For rotations outside the interval \(\alpha_1 < \alpha < \alpha_2\), the spring response follows the enhanced torsional spring law introduced in the previous section. The bistable law therefore governs the interior response, while the enhanced law controls the limiting behavior near the admissible rotation bounds.
